...
func voteRoutes() {
	r.GET("/:candidate", authMiddleware(), func(c *gin.Context) {
		...
		video := ""
		err := dbpool.QueryRow(context.Background(), "SELECT * FROM candidates WHERE name = $1", name).Scan(&userId, &name, nil, &description, &hookstatement, &video, &keywords, &positions)
		if err != nil {
